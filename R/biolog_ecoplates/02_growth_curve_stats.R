...
readr::write_rds(many_spline, here::here(data, "gcurves_spline_fits"))
...
many_spline_fitted <- purrr::map(many_spline@fits, \(x) {
  ...
  dplyr::left_join(gcurves_sm, by = dplyr::join_by(id, hours)) %>%
  dplyr::group_by(id) %>%
...
many_auc_res <- gcurves_sm %>%
  ...
  dplyr::left_join(
    dplyr::distinct(dplyr::select(gcurves_sm, strain:id)),
    by = join_by(id)
  ) %>%
  dplyr::select(-id)
...
readr::write_tsv(
  many_auc_res,
  here::here(data, "gcurves_auc_results.tsv")
)
...
many_spline_res %>%
  ...
  dplyr::select(-id) %>%
  readr::write_tsv(here::here(data, "gcurves_spline_results.tsv"))
