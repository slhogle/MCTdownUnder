...
plotplate <- function(
  ...
  xyfilt <- if (!is.null(dfxy)) {
    inner_join(
      dfxy,
      distinct(dffilt, well, plate_name),
  ...
  ggplot(dffilt, aes(x = hours)) +
    list(
      ...
      if (unsmoothed) {
        ggplot2::geom_line(aes(y = OD600), color = "orange", lty = 2)
      },
      if (predicted) {
        ggplot2::geom_line(aes(y = predicted), color = "orange")
      },
      if (!is.null(dfxy)) {
        ggplot2::geom_point(
          data = xyfilt,
...
read_logphase_xlsx <- function(subdir, filename, sheet, skip) {
  readxl::read_xlsx(
    here::here(data_raw, subdir, filename),
    sheet = sheet,
    skip = skip
  ) %>%
